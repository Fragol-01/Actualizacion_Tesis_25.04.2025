...
void DS3231_Set_Date_Time(uint8_t dy, uint8_t mth, uint8_t yr, uint8_t dw, uint8_t hr, uint8_t mn, uint8_t sc)
{
	...
	HAL_I2C_Master_Transmit(&hi2c1, (uint16_t)DS3231_ADDRESS, data_tx, 8, 100);
}
...
{
	data_tx[0] = reg;
	HAL_I2C_Master_Transmit(&hi2c1, (uint16_t)DS3231_ADDRESS, &data_tx[0], 1, 100);
	HAL_I2C_Master_Receive(&hi2c1, (uint16_t)DS3231_ADDRESS, data_rx, 1, 100);
    return data_rx[0];
}

/*
 * @brief transform binary to BCD
 */
